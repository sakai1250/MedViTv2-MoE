...
from MedViT_origin import MedViT_tiny, MedViT_small, MedViT_base, MedViT_large
...
def main(args):
    ...
    if model_name in model_classes:
        ...
        if args.sr_ratios:
            # Parse sr_ratios from string "[8, 4, 2, 1]" or "8,4,2,1"
            import ast
            try:
                sr_ratios = ast.literal_eval(args.sr_ratios)
                if not isinstance(sr_ratios, list):
                     # Fallback if it's not a list representation
                     sr_ratios = [int(x) for x in args.sr_ratios.split(',')]
            except:
                 sr_ratios = [int(x) for x in args.sr_ratios.split(',')]
            
            print(f"Using sr_ratios: {sr_ratios}")
            net = model_class(num_classes=nb_classes, use_kan=args.use_kan, sr_ratios=sr_ratios).cuda()
        else:
            net = model_class(num_classes=nb_classes, use_kan=args.use_kan).cuda()
        if pretrained:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sr_ratios', type=str, default=None, help='Spatial reduction ratios (e.g., "[8, 4, 2, 1]").')
