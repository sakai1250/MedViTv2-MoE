...
run_ablation() {
    local dataset=$1
    local gpu_id=$2
    local output_file="${dataset}_ablation_result_medv2.md"

    echo "Running Gabor-KAN Ablation (MedViTv2) for $dataset on GPU $gpu_id"

    # Write header for Markdown table
    echo "## Gabor-KAN Ablation Study Results - MedViTv2 ($dataset)" > "$output_file"
    ...
    CUDA_VISIBLE_DEVICES=$gpu_id python main.py --model_name 'MedViT_tiny' --dataset "$dataset" --pretrained True --use_ackan True --ackan_stages 0 1 2 3 --epochs 100
    extract_and_log "All Stages [0, 1, 2, 3]"

    echo "=== 2. Shallow Stages Only [0, 1] ==="
    CUDA_VISIBLE_DEVICES=$gpu_id python main.py --model_name 'MedViT_tiny' --dataset "$dataset" --pretrained True --use_ackan True --ackan_stages 0 1 --epochs 100
    extract_and_log "Shallow Stages [0, 1]"

    echo "=== 3. Deep Stages Only [2, 3] ==="
    CUDA_VISIBLE_DEVICES=$gpu_id python main.py --model_name 'MedViT_tiny' --dataset "$dataset" --pretrained True --use_ackan True --ackan_stages 2 3 --epochs 100
    extract_and_log "Deep Stages [2, 3]"

    echo "=== 4. No ACKAN (Standard Conv/MLP) ==="
    CUDA_VISIBLE_DEVICES=$gpu_id python main.py --model_name 'MedViT_tiny' --dataset "$dataset" --pretrained True --epochs 100
