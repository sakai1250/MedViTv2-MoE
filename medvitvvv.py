...
MedViTVVV: A Robust Vision Transformer for Generalized Medical Image Classification with FM-KAN.
(Frequency Modulation KAN based variant)
"""
from functools import partial
import math
from fm_kan import FMKAN as KAN
import torch
...
class LFP(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, path_dropout=0.2,
        ...
        self.attn = nn.Identity() # Fallback if natten is not available
...
class GFP(nn.Module):
    """
    Local Transformer Block with FM-KAN
    """
    def __init__(
        ...
        # Use FM-KAN instead of ORKAN/FasterKAN
...
class MedViTVVV(nn.Module):
    """
    MedViTVVV: MedViT with FM-KAN (Frequency Modulation KAN)
    """
    def __init__(self, stem_chs=[64, 32, 64], depths=[2, 2, 6, 2],
                 ...
                 use_checkpoint=False, use_kmp_glu=False, use_coord=False, **kwargs):
        super(MedViTVVV, self).__init__()
...
@register_model
def MedViTVVV_tiny(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay= None, **kwargs):
    model = MedViTVVV(stem_chs=[64, 32, 64],
                   depths=[2, 2, 6, 1],
                   dims=[64, 128, 192, 384],
...
@register_model
def MedViTVVV_small(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay= None, **kwargs):
    model = MedViTVVV(stem_chs=[64, 32, 64],
                   depths=[2, 2, 6, 2],
                   dims=[64, 128, 256, 512],
                   path_dropout=0.1, **kwargs)
...
@register_model
def MedViTVVV_base(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay= None, **kwargs):
    model = MedViTVVV(stem_chs=[64, 32, 64],
                   depths=[2, 2, 6, 2],
                   dims=[96, 192, 384, 768],
...
@register_model
def MedViTVVV_large(pretrained=False, pretrained_cfg=None, pretrained_cfg_overlay= None, **kwargs):
    model = MedViTVVV(stem_chs=[64, 32, 64],
                   depths=[2, 2, 6, 2],
