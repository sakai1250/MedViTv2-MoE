...
class GFP(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_kmp_glu:
            ...
            stats = torch.cat([gap, var], dim=1)
            # alpha = torch.sigmoid(self.gate(stats)).view(b, d, 1, 1)
            alpha = torch.ones(gap.size(0), gap.size(1), 1, 1, device=gap.device)
            if self.training:
                self._update_stats(
                    alpha_mean=alpha.detach().mean().item(),
                    gap_mean=gap.detach().mean().item(),
                    var_mean=var.detach().mean().item(),
                )
            fused = alpha * kan_out + (1.0 - alpha) * mlp_out
...
class MedViT(nn.Module):
    def __init__(self, stem_chs=[64, 32, 64], depths=[2, 2, 6, 2],
                 ...
                 use_checkpoint=False, use_kmp_glu=False):
            ...
            for block_id in range(numrepeat):
                ...
                if block_type is LFP:
                    ...
                    layer = GFP(input_channel, output_channel, path_dropout=dpr[idx + block_id], stride=stride,
                                ...
                                attn_drop=attn_drop, drop=drop, use_kmp_glu=self.use_kmp_glu)
                    features.append(layer)
